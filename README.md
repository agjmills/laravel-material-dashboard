...
3. Publish the public assets:
    ...
    ```
    php artisan vendor:publish --provider="Agjmills\LaravelMaterialDashboard\ServiceProvider" --tag=assets
    ```
...
To use the template, create a blade file and extend the layout with `@extends('material-dashboard::page')`.
...
@extends('material-dashboard::page')
...
## 4. The `make:material-dashboard` artisan command

> Note: only for Laravel 5.2 and higher

This package ships with a `make:material-dashboard` command that behaves exactly like `make:auth` (introduced in Laravel 5.2) but replaces the authentication views with Material Dashboard style views.

```
php artisan make:material-dashboard
```

This command should be used on fresh applications, just like the `make:auth` command

### 4.1 Using the authentication views without the `make:material-dashboard` command
...
```
@extends('material-dashboard::login')
```
- `resources/views/auth/register.blade.php`
```
@extends('material-dashboard::register')
```
- `resources/views/auth/passwords/email.blade.php`
```
@extends('material-dashboard::passwords.email')
```
- `resources/views/auth/passwords/reset.blade.php`
```
@extends('material-dashboard::passwords.reset')
```
...
```
php artisan vendor:publish --provider="Agjmills\LaravelMaterialDashboard\ServiceProvider" --tag=config
```

Now, edit `config/material-dashboard.php` to configure the title, skin, menu, URLs etc. All configuration options are explained in the comments. However, I want to shed some light on the `menu` configuration.
...
use Agjmills\MaterialDashboard\Menu\Filters\FilterInterface;
use Laratrust;
...
And then add to `config/material-dashboard.php`:

```php
'filters' => [
    Agjmills\LaravelMaterialDashboard\Menu\Filters\ActiveFilter::class,
    Agjmills\LaravelMaterialDashboard\Menu\Filters\HrefFilter::class,
    Agjmills\LaravelMaterialDashboard\Menu\Filters\SubmenuFilter::class,
    Agjmills\LaravelMaterialDashboard\Menu\Filters\ClassesFilter::class,
    //Agjmills\LaravelMaterialDashboard\Menu\Filters\GateFilter::class, Comment this line out
...
By default the [DataTables](https://datatables.net/) plugin is supported. If set to `true`, the necessary javascript CDN script tags will automatically be injected into the `material-dashboard::page.blade` file.
...
Also the [Select2](https://select2.github.io/) plugin is supported. If set to `true`, the necessary javascript CDN script tags will automatically be injected into the `material-dashboard::page.blade` file.
...
Also the [ChartJS](https://www.chartjs.org/) plugin is supported. If set to `true`, the necessary javascript CDN script tags will automatically be injected into the `material-dashboard::page.blade` file.
...
```
php artisan vendor:publish --provider="Agjmills\LaravelMaterialDashboard\ServiceProvider" --tag=views
```

Now, you can edit the views in `resources/views/vendor/material-dashboard`.
